...
// Page-specific CSS for organizer dashboard
if (!isset($extra_css) || !is_array($extra_css)) { $extra_css = []; }
$extra_css[] = 'organizer_dashboard';
...
<section class="py-5" style="min-height:90vh;">
    <div class="container">
        ...
        <h4 class="mb-3 mt-4 text-center"><?php echo ($lang == 'ar') ? 'فعالياتي المُقدمة' : 'My Submitted Events'; ?></h4>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                            ...
                            <td><?php echo format_date($event['start_at']); ?></td>
                            
                            <td>
                                <?php
                                    // Map approval_status to label text (localized)
                                    ...
                                    if (($lang ?? 'en') === 'ar') {
                                        switch ($status) {
                                            case 'approved': $statusLabel = 'مقبول'; break;
                                            case 'rejected': $statusLabel = 'مرفوض'; break;
                                            default: $statusLabel = 'قيد الانتظار'; break;
                                        }
                                    } else {
                                        // English: human-friendly
                                        switch ($status) {
                                            case 'approved': $statusLabel = 'Approved'; break;
                                            case 'rejected': $statusLabel = 'Rejected'; break;
                                            default: $statusLabel = 'Pending'; break;
                                        }
                                    }
                                ?>
                                <span class="badge <?php echo $badgeClass; ?>"><?php echo e($statusLabel); ?></span>
                            </td>
                        <td>
                            <!-- ✅ التصحيح الحرج: تغيير ? إلى & لربط ID بـ page -->
                            <a href="<?php echo BASE_URL; ?>/?page=event_detail&id=<?php echo $event['id']; ?>" 
                               class="btn btn-sm action-btn action-eye me-2" 
                               title="<?php echo ($lang == 'ar') ? 'التفاصيل' : 'Details'; ?>">
                                <i class="bi bi-eye"></i>
                            </a>
                            <a href="edit_event.php?id=<?php echo $event['id']; ?>" class="btn btn-sm action-btn action-edit me-2" title="<?php echo ($lang == 'ar') ? 'تعديل' : 'Edit'; ?>"> 
